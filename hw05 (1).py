...
def find(t, target):
    ...
    if is_leaf(t) == True:
        return False
    results = [find(i, target) for i in branches(t)]
    if True in results:
        return True
    else:
        return False
...
def add_song(t, song, category):
    ...
    eachbran = []
    for i in branches(t):
        eachbran = eachbran + [add_song(i, song, category)]
    return tree(root(t), eachbran)
...
def make_withdraw(balance, password):
    ...
    """
    guesses = []
    def withdraw(amount, answer):
        nonlocal balance
        if len(guesses) == 3:
            return 'Your account is locked. Attempts: ' + str(guesses[:3])
        if answer != password and len(guesses) < 3:
            guesses.append(answer)
            return 'Incorrect password'
        if amount > balance and answer == password:
